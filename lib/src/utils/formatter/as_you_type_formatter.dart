...
class AsYouTypeFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if (newValueLength > 0 && newValueLength > oldValueLength) {
      ...
      final _ = newValueText.substring(oldValue.selection.start == -1 ? 0 : oldValue.selection.start,
              newValue.selection.end == -1 ? 0 : newValue.selection.end).replaceAll(separatorChars, '');
      ...
      formatAsYouType(input: textToParse).then((String? value) {
          ...
          if (separatorChars.hasMatch(parsedText)) {
            ...
            onInputFormatted(TextEditingValue(text: parsedText,
  ...
  String parsePhoneNumber(String? phoneNumber) {
    if (dialCode.length > 4) {
      if (isPartOfNorthAmericanNumberingPlan(dialCode)) {
        ...
        return phoneNumber!.replaceFirst(countryDialCodeWithSpace, '').replaceFirst(separatorChars, '').trim();
      }
    }
